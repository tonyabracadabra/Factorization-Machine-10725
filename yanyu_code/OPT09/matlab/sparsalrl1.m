...
%
% arg min_x = sum(log(1+exp(-y.*(A*x)))) + tau phi(x)
%
...
if (sum(stopCriterion == [0 1 2 3 4 5 6])==0)
...
while keep_continuation 
  ...
  yz = A(x).*y; yzm = max(0,yz);
  resid =  -y.*exp(-yzm)./(exp(-yzm)+exp(yz-yzm));
  gradq = AT(resid);
  ...
  % for this tau
  alpha = 1/eps;
  f = loss_lrp(yz) + tau * phi_function(x);
  if enforceSafeguard
    f_lastM = f;
  end
  ...
  while keep_going
    ...
    switch stopCriterion
        ...
        case 2,
            ...
            ya = -tau/scaleFactor*(y.*prev_resid(:));
            criterionDuality = loss_lrp(prev_yz) + ...
                tau * phi_function(prev_x) + loss_lrd(ya);
        ...
        case 6,
            constraintDual = norm(gradq(:),inf)/tau-1;
            criterionDuality = gradq(:)'*x(:)+tau * phi_function(prev_x);
            criterionDuality = abs(criterionDuality) / max(1,prev_f);
            keep_going = max(constraintDual,criterionDuality) > tolA;
            if verbose
               fprintf('dc=%e gap=%e (target=%e)\n',constraintDual, ...
                       criterionDuality, tolA);
            end
        otherwise,
            error(['Unknown stopping criterion']);
...
if (debias & (sum(x(:)~=0)~=0))
  error('No debiassing implemented!');
end
...
function f = loss_lrp(yz)
yzm = max(0,yz);
f = sum(-yz+yzm+log(exp(-yzm)+exp(yz-yzm)));

function d = loss_lrd(ya)

I = find(0<ya & ya<1);
d = sum((1-ya(I)).*log(1-ya(I))+ya(I).*log(ya(I)));
