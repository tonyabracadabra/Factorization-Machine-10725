...
\begin{align}
  L(w_0, w, V)/L(w_0, w, W) = \sum_{i = 1}^n (y^i - \hat{y}(x^i, w_0, w, V)^2 \label{eq:criteria}
\end{align}
...
\begin{cases}
  ...
  \end{cases}
\end{align*}
Since the only non-smooth term is $\|w\|_1$, then we can optimize the criteria $L(w_0, w, V)$ with proximal gradient descent and accelerated proximal method. Besides these, we plan to try method with superlinear convergence, i.e. Newton’s method and quasi-Newton method. The hessian of loss function is:
\begin{align*}
